...
void EnemyI::itemAffect(const int itemSelected) {
	if (itemSelected == 1) {  // slowing spell
		...
		for (int i = 0; i < healthEnemyS.size(); i++) {
			...
			for (int i = 0; i < enemyS.size(); i++) {
				dmg.push_back(std::make_pair(tempDmg, enemyS[i].getPosition().y + imageSize.y / 2 * scale.y));
				dmg.back().first.setString("20");
				dmg.back().first.setPosition(enemyS[i].getPosition().x + imageSize.x / 2 * scale.x - 20, enemyS[i].getPosition().y + imageSize.y / 2 * scale.y);
